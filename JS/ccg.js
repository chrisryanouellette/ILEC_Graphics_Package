...
const ccg = (function() {
    ...
    async function update(raw) {
        ...
        if(!SERVER.isAuthenitcated) {
            ...
            try {
                ...
                }).then(() => attemptWSConnection({
                    URL: parsed.server.WSURL, 
                    PATH: parsed.server.WSPATH, 
                    playoutInfo: parsed.playoutInfo
                })).catch(error => {
                    throw new Error(error);
                });
            } catch (error) {
                if(parsed.server.BYPASS || graphic.ENV === 'DEV') {
                    delete parsed.server.BYPASS;
                } else {
    ...
    */
    ...
    function attemptWSConnection({URL, PATH, playoutInfo}) {
        return new Promise((resolve, reject) => {
            try {
                ...
                // Attempt to connect
                SERVER.ws = new WebSocket(SERVER.WS_URL + `?channel=${playoutInfo.channel}&layer=${playoutInfo.layer}`, 
                    [document.cookie.replace(/(?:(?:^|.*;\s*)connect.sid\s*\=\s*([^;]*).*$)|^.*$/, "$1")]);
